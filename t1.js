...
// // (function anonymous(I,runner,hook,ll,lll,a,b) {
...
//     "if (!f.size) {continue;}",
//     "for (const g of f) {",
//     "I.push({i: g, iiii(ii) {const iii=ii.replace(lll, '$1');if (iii && g.includes(iii)) {this.iii = createNode(); this.iii.href = g; return true;}},});}}setTimeout(() => { runner(); hook.reset(); }, defaultDelay);};}",
// ];
// const c = JSON.parse(b);
// if (c.code == 200) {
//     if (c.data.hidden_content?.locked) { targets[0] = { a: '/getPostHiddenContent', b: respHandler }; return; }
